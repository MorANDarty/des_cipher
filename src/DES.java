...
public class DES {
    ...
    // Первый индекс - это значение мусора, циклы, работающие с этим, должны начинаться с index = 1
    ...
    /**
     * Зашифровывает строковое сообщение блочным шифром DES
     * @param key - ключ
     * @param plaintext - сообщение строковое
     */
    public String encrypt(String key, String plaintext) {
        ...
        // разделяем текст на бинарные блоки
        ...
        // Создаем двоичную строку зашифрованного текста из блоков
    ...
    public String decrypt(String key, String plaintext) {
        ...
        // разделяем бинарный текст на блоки
        String[] binPlaintextBlocks = new String[binPlaintext.length()/64];
        int offset = 0;
        for (int i = 0; i < binPlaintextBlocks.length; i++) {
            binPlaintextBlocks[i] = binPlaintext.substring(offset, offset+64);
        ...
        // строим зашифрованную строку из блоков
        String binCiphertext = "";
        for (int i = 0; i < binCiphertextBlocks.length; i++)
            binCiphertext += binCiphertextBlocks[i];
    ...
    public String encryptBlock(String plaintextBlock) throws Exception {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            // 48-битный текущий ключ
            String curKey = Long.toBinaryString(K[i+1]);
            while(curKey.length() < 48)
                curKey = "0" + curKey;

            // Получаем 32 битный результат из f с m1 и ki
    ...
    public String decryptBlock(String plaintextBlock) throws RuntimeException {
        ...
        for (int i = 16; i > 0; i--) {
            ...
            // получаем 32 битный в результате f с m1 и ki
            String fResult = f(mR, curKey);
    ...
    public void buildKeySchedule(long key) {
        ...
        // Добавляем начальные нули, если длина ключа не указана, для простоты вычислений
        while (binKey.length() < 64)
            binKey = "0" + binKey;

        // Для 56-битного переставленного ключа
        String binKey_PC1 = "";
        ...
        // Разбиваем перестановочную строку пополам | 56/2 = 28
        sL = binKey_PC1.substring(0, 28);
        sR = binKey_PC1.substring(28);

        // Разбираем двоичные строки в целые числа для сдвига
        iL = Integer.parseInt(sL, 2);
        iR = Integer.parseInt(sR, 2);

        // Строим ключи, начинаем с индекса 1
        for (int i = 1; i < K.length; i++) {

            // Сдвиг влево в соответсвии с массивом сдвига
            ...
            // исправляем длину если ведущие нули отсутствуют
    ...
    public static String f(String mi, String key) {
        ...
        // XOR расширенного блока сообщений и блока ключей (48 бит)
        Long result = m ^ k;

        String bin = Long.toBinaryString(result);
        // убеждаемся что строка содержит 48 бит
        while (bin.length() < 48) {
            bin = "0" + bin;
        }

        // Разделяем на восемь 6-битных строк
        ...
        for (int i = 0 ; i < 8; i++) {
            ...
            // убеждаемся что строка из 4 битов
            while(sout[i].length() < 4)
                sout[i] = "0" + sout[i];

        }

        // Соединяем S-Box результаты в одну 32 битную строку
        String merged = "";
        for (int i = 0; i < 8; i++) {
    ...
    public static void main(String[] args) {
        ...
        if (enc) {
            if (message == null) {
                System.out.println("Не дали строку");
                System.exit(0);
            } else if (key1 == null) {
                System.out.println("Неправильное использование ключевых аргументов");
                System.exit(0);
            }
            ...
            if (message == null) {
                System.out.println("Не дали данных для дешифровании");
                System.exit(0);
            } else if (key1 == null) {
                System.out.println("Неправильное использование ключевых аргументов");
                System.exit(0);
